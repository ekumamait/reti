...
export const handleDownloadData = (
  data: any,
  options?: { mode?: "download" | "blob" }
) => {
  if (!data) {
    toast.error("Unable to generate PDF. No data available.");
    return;
  }
  ...
  const lightGrayColor = [189, 195, 199];

  doc.setTextColor(secondaryColor[0], secondaryColor[1], secondaryColor[2]);

  const headerColWidth = (pageWidth - 2 * margin) / 3;

  const leftColWidth = pageWidth * 0.4;
  ...
  const addLabelValue = (
    ...
    if (lines.length === 1) {
      ...
      doc.text(lines, x, y + fontSize * 0.4);
      return y + lines.length * fontSize * 0.4 + fontSize * 0.4;
    }
  };
  ...
  const addFooter = () => {
    ...
    doc.line(
      ...
      const logoHeight = 15;
      const columns = 5;
      const spacingX = (pageWidth - 2 * margin) / columns;
      const spacingY = 20;
  ...
  if (userProfile?.profileImage) {
    ...
    doc.roundedRect(imageX, headerY, imageSize, imageSize, 25, 25, "F");
    doc.addImage(
  ...
  if (userProfile?.skills && userProfile.skills.length > 0) {
    ...
    const darkGrayColor = [150, 150, 150];
    const tabPadding = 4;
    const tabSpacing = 4;
