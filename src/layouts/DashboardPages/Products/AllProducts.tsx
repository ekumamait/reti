import { ClockCircleOutlined, MoneyCollectOutlined, InboxOutlined } from "@ant-design/icons";
...
const AllProductsPage = ({
	...
	if (!products?.data || products.data.length === 0) {
		return (
			<EmptyState
				...
				icon={<InboxOutlined style={{ fontSize: '2rem' }} />}
				className="mt-8"
			/>
		);
	}
	...
	if (filteredProducts.length === 0) {
		return (
			<EmptyState
				message={`No products found ${searchText ? 'matching your search' : ''} ${stockFilter !== 'all' ? `in ${stockFilter} category` : ''}`}
				icon={<InboxOutlined style={{ fontSize: '2rem' }} />}
				className="mt-8"
			/>
		);
					...
					>
						<div className="p-4">
							...
							<div className="flex space-x-4">
								<div className="w-1/3">
									<img
										src={product?.imageUrl?.[0] || 'https://via.placeholder.com/300x200'}
										alt={product.name}
