...
const RetiCandidatePage = ({ form, formData, setFormData }) => {
  const handleRetiCandidateClick = (value: boolean) => {
    setFormData((prev) => ({
      ...prev,
      isRetiCandidate: value,
      retiPartner: value ? prev.retiPartner : null,
    }));
    form.setFieldsValue({
      isRetiCandidate: value,
      retiPartner: value ? form.getFieldValue("retiPartner") : null,
    });
  };
      ...
      {form.getFieldValue("isRetiCandidate") && (
        <div className="space-y-6">
          ...
          <Form.Item
            name="retiPartner"
            rules={[
              { required: true, message: "Partner selection is required" },
            ]}
          >
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {partners.map((partner) => (
                <div
                  ...
                  className={`p-4 border rounded-lg cursor-pointer h-full transition-colors
                                    ${
                                      form.getFieldValue("retiPartner") ===
                                      partner
                                        ? "border-blue-500 bg-blue-50"
                                        : "hover:border-blue-500"
                                    }`}
